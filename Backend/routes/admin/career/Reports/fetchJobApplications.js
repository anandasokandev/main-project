...
router.post('/', (req, res, next) => {
  ...
  let {status , startDate, endDate, disabilitySubCategory, disabilityCategory, job} = req.body;
  ...
  if (status) {
    query += ' AND jf.form_status = ?'; // Using LIKE for partial matching
    queryParams.push(status); // Adding the search term with wildcards for partial matching
  }
